...
describe('createApp plumbing', () => {
  ...
  it('GET /api/health is public and does not require auth', async () => {
    const res = await request(await createApp()).get('/api/health');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ status: 'ok' });
  });

  it('GET /api/ready is public and checks the database', async () => {
    const res = await request(await createApp()).get('/api/ready');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ status: 'ready' });
  });

  it('rejects protected routes without a bearer token', async () => {
    const res = await request(await createApp()).get('/api/does-not-exist-yet');
